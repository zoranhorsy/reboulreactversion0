...
exports.SneakersProductController = {
  async createSneakersProduct(data, files) {
    ...
    const result = await db.query(
      `INSERT INTO sneakers_products (name, price, description, category_id, brand_id, brand, image_url, images, variants, tags, details, featured, active, new, sku, store_reference, material, weight, dimensions)
  ...
  async updateSneakersProduct(id, data, files) {
    ...
    values.push(id);
    const result = await db.query(
      `UPDATE sneakers_products SET ${fields.join(', ')} WHERE id = $${idx} RETURNING *`,
      values
    );
    return result.rows[0];
  }
