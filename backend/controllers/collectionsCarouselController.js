...
const createCollection = async (req, res) => {
  try {
    ...
    const result = await db.query(query, [name, description, image_url, link_url, badge, sort_order || 0]);
...
const updateCollection = async (req, res) => {
  try {
    ...
    const result = await db.query(query, [name, description, image_url, link_url, badge, sort_order, is_active, id]);
  ...
  try {
    ...
    for (const collection of collections) {
      const query = `
        ...
        WHERE id = $2
      `;
      await db.query(query, [collection.sort_order, collection.id]);
    }
