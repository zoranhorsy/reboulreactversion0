...
export const getPublicIdFromCloudinaryUrl = (url: string): string => {
  ...
  if (url.includes('cloudinary.com')) {
    console.log('URL Cloudinary complète détectée, retournée telle quelle:', url);
    return url;
  }
  ...
  try {
    ...
    const transformationIndex = relevantParts.findIndex(part => part.includes(','));
    ...
    if (!url.startsWith('http') && url.includes('/')) {
      console.log('Après erreur, URL considérée comme un publicId direct:', url);
      return url;
    }
    ...
    console.log('Toutes les tentatives d\'extraction ont échoué, retour de l\'URL complète');
    return url;
  }
};
