 ...
 */
export function getVercelOptimizedUrl(src: string, options: ImageOptimizationOptions = {}): string {
 ...
 */
export function getCloudinaryOptimizedUrl(src: string, options: ImageOptimizationOptions = {}): string {
 ...
 */
export function getLocalOptimizedUrl(src: string, options: ImageOptimizationOptions = {}): string {
 ...
 */
export function getImageSources(src: string, widths: number[] = [640, 750, 828, 1080, 1200, 1920]): {
  webpSrcSet: string;
  avifSrcSet: string;
  defaultSrc: string;
} {
  ...
  if (isExternalUrl(src)) {
    if (src.includes('res.cloudinary.com')) {
      return {
        webpSrcSet: widths.map(w => `${getCloudinaryOptimizedUrl(src, { width: w, format: 'webp', imageType })} ${w}w`).join(', '),
        avifSrcSet: widths.map(w => `${getCloudinaryOptimizedUrl(src, { width: w, format: 'avif', imageType })} ${w}w`).join(', '),
        defaultSrc: getCloudinaryOptimizedUrl(src, { format: 'auto', imageType })
      };
    }
  ...
  // Pour les images locales, utiliser le CDN Vercel
  return {
    webpSrcSet: widths.map(w => `${getVercelOptimizedUrl(src, { width: w, format: 'webp', imageType })} ${w}w`).join(', '),
    avifSrcSet: widths.map(w => `${getVercelOptimizedUrl(src, { width: w, format: 'avif', imageType })} ${w}w`).join(', '),
