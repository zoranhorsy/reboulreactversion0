...
export class ProductService {
  ...
  async fetchProducts(
    ...
    try {
      ...
      if (!data.pagination || typeof data.pagination.totalItems !== "number") {
        throw new Error(
          "Invalid response format: pagination.totalItems should be a number",
        );
      }
  ...
  async searchProducts(query: string): Promise<Product[]> {
    try {
      const response = await this.instance.get(`/products/search`, {
        params: { q: query },
      });
      ...
      if (!Array.isArray(data)) {
        throw new Error(
          "Invalid response format: expected an array of products",
        );
      }
  ...
  async updateProductStock(id: string, stock: number): Promise<Product> {
    try {
      const response = await this.instance.put(`/products/${id}/stock`, {
        stock,
      });
      return response.data;
    } catch (error) {
