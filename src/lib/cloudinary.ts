...
export const uploadImages = async (
  ...
  try {
    console.log(
      "Début upload multiple vers Cloudinary...",
      files.length,
      "fichiers",
    );
    ...
    if (files.length > 1) {
      ...
      const data = await response.json();
      console.log(
        "Images uploadées avec succès (méthode FormData):",
        data.urls,
      );
      ...
      const results = data.urls.map((url: string) => {
        const publicId =
          data.public_ids?.[data.urls.indexOf(url)] || getPublicIdFromUrl(url);
        return { url, publicId };
      });
    ...
    // Méthode 2: Upload individuel (pour un seul fichier)
      ...
      console.log(
        "Images uploadées avec succès (méthode individuelle):",
        results,
      );
      return results;
    }
...
export const fixCloudinaryUrl = (url: string): string => {
  ...
  if (url.includes("cloudinary.com")) {
    ...
    if (!url.includes("/upload/")) {
      ...
      const cloudName =
        process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || "dxen69pdo";
  ...
  if (
    !url.startsWith("http") &&
    (url.includes("/") || url.includes("reboul-store"))
  ) {
    const cloudName =
      process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || "dxen69pdo";
...
export const cloudinaryUploadImages = async (
  files: File[],
): Promise<CloudinaryUploadResult[]> => {
  try {
