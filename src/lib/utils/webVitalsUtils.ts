...
): Array<{ metric: WebVitalName, value: number, threshold: number, pagePath?: string }> {
  const issues = []
...
export function generateRecommendations(
  issues: Array<{ metric: WebVitalName, value: number, threshold: number, pagePath?: string }>
): string[] {
  ...
  for (const issue of issues) {
    switch (issue.metric) {
      case 'LCP':
        recommendations.add('Optimiser les images principales avec préchargement prioritaire')
        recommendations.add('Améliorer le temps de réponse du serveur ou utiliser un CDN')
        recommendations.add('Minimiser le CSS bloquant le rendu')
        break
      
      case 'CLS':
        recommendations.add('Spécifier les dimensions des images et médias')
        recommendations.add('Réserver l\'espace pour les éléments dynamiques (publicités, embeds)')
        recommendations.add('Éviter d\'insérer du contenu au-dessus du contenu existant')
        break
      
      case 'FID':
      case 'INP':
        recommendations.add('Réduire le temps d\'exécution JavaScript')
        recommendations.add('Décomposer les tâches longues en tâches plus courtes')
        recommendations.add('Différer ou supprimer le JavaScript non essentiel')
        recommendations.add('Optimiser les gestionnaires d\'événements pour réduire la latence')
        break
      
      case 'TTFB':
        recommendations.add('Optimiser le serveur (mise en cache, optimisation des requêtes)')
        recommendations.add('Utiliser un CDN pour les ressources statiques')
        recommendations.add('Optimiser le temps de génération des pages côté serveur')
        break
