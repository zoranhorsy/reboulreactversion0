...
export function LazyLoadOnView<T extends JSX.IntrinsicAttributes>({
    ...
    if (inView && !Component && !loading) {
      ...
      importFn()
        .then((module) => {
          ...
          if (debug)
            console.log("LazyLoadOnView: Composant chargé avec succès");
        })
        .catch((err) => {
          setError(err);
          if (debug)
            console.error("LazyLoadOnView: Erreur lors du chargement", err);
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  if (error) {
    return (
      <div
        className={`text-red-500 p-4 border border-red-200 rounded-md ${className}`}
      >
