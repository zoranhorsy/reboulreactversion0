    ...
    const calculateRecommendation = () => {
      ...
      if (isKidsSizes && height !== "") {
        ...
        for (let i = 0; i < sizeChart.length; i++) {
          ...
          if (!nextSize || heightSize <= size.chest) {
            // Utiliser chest comme approximation de la hauteur
            return currentSize;
          }
        }
  ...
  if (isLoading) {
    return (
      <div className="py-4 text-center text-muted-foreground">
        Chargement du guide des tailles...
      </div>
    );
  }
    ...
    <div className="space-y-4">
      {!showResult ? (
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div
              className={`grid grid-cols-1 ${isKidsSizes ? "sm:grid-cols-3" : "sm:grid-cols-3"} gap-3`}
            >
              {isKidsSizes && (
            ...
            <Button
              ...
              className="w-full"
              disabled={
                (isKidsSizes && !height && !chest && !waist) ||
        ...
        <Card className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-muted p-3 rounded-lg flex flex-col items-center justify-center">
              ...
              {recommendedSize !== "Taille non trouvée" &&
                recommendedSize !== "Mesures insuffisantes" && (
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-md p-3 text-xs text-blue-600">
            <span>ℹ️</span>
            Cette recommandation est basée sur des mesures standard. Pour un
            ajustement parfait, essayez le produit ou consultez notre service
            client.
          </div>
        </Card>
      )}
      ...
      <div className="overflow-x-auto mt-5">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>{isKidsSizes ? "ÂGE" : "TAILLE"}</TableHead>
              <TableHead>
                {isKidsSizes ? "HAUTEUR (cm)" : "POITRINE (cm)"}
              </TableHead>
              <TableHead>TAILLE (cm)</TableHead>
              {!isKidsSizes && <TableHead>HANCHES (cm)</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {sizeChart.map((size) => (
              <TableRow key={size.size}>
                ...
                <TableCell>{isKidsSizes ? size.chest : size.chest}</TableCell>
                <TableCell>{size.waist}</TableCell>
                {!isKidsSizes && <TableCell>{size.hips}</TableCell>}
              </TableRow>
            ))}
          </TableBody>
        </Table>
