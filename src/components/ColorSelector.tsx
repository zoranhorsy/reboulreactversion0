  ...
  return (
    <div className="mb-4 sm:mb-6 relative">
      ...
      {hoveredColor && productImages[hoveredColor] && (
        <div className="hidden sm:block absolute right-0 -top-16 w-20 h-20 border border-zinc-300 rounded-md overflow-hidden shadow-lg z-10 bg-white">
          <Image
        ...
        {colors.map((color) => {
          ...
          return (
            <button
              ...
              disabled={!available}
              className={cn(
                "relative px-2 py-2 sm:px-3 sm:py-2.5 rounded-md flex items-center justify-center",
                "transition-all duration-200 border min-w-[60px] sm:min-w-[80px]",
                "text-xs sm:text-sm",
                selectedColor === color
