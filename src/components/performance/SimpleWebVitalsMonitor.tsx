...
import { AlertCircle, AlertTriangle, CheckCircle, BarChart } from 'lucide-react'
...
export function SimpleWebVitalsMonitor() {
  ...
  useEffect(() => {
    ...
    const collectMetrics = async () => {
      try {
        ...
        if (mounted) {
          setMetrics(prev => {
            const newMetrics = prev.filter(m => m.name !== 'CLS' && m.name !== 'FID')
            return [
              ...newMetrics, 
              {
                name: 'CLS',
        ...
        console.error('Erreur lors de la collecte des métriques Web Vitals:', error)
      }
    }
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50">
      ...
      >
        {icon}
        <span className="text-white text-xs font-medium">Performance: {score}</span>
      </Button>
    </div>
  )
} 