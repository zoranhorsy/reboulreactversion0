...
export function WebVitalsMonitor() {
  ...
  const [currentTab, setCurrentTab] = useState<"metrics" | "workers">(
    "metrics",
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "Erreur lors de l'enregistrement des métriques Web Vitals",
        e,
      );
    }
    ...
    return () => {
      ...
      metricsReporters.forEach((unsubscribe) => {
        if (typeof unsubscribe === "function") {
          try {
            ...
            console.error(
              "Erreur lors du désabonnement des métriques Web Vitals",
              e,
            );
          }
        }
      ...
      try {
        ...
        console.error(
          "Erreur lors de la suppression de l'écouteur de performance",
          e,
        );
      }
  ...
  useEffect(() => {
    ...
    const handlePerformanceEntry = (entries: PerformanceEntry[]) => {
      for (const entry of entries) {
        ...
        if (entry.entryType === "measure" && entry.name.startsWith("worker-")) {
    ...
    const testWithWithoutWorkers = async () => {
      try {
        ...
        performance.measure("filter-with-worker", {
          ...
          detail: {
            count: Array.isArray(filterResult) ? filterResult.length : 0,
          },
        });
        ...
        result = result.filter((product) => product.category === "Chaussures");
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <PopoverContent>
          <Tabs
            value={currentTab}
            onValueChange={(value) =>
              setCurrentTab(value as "metrics" | "workers")
            }
          >
            ...
            <TabsContent value="metrics">
              ...
              <div className="mb-4">
                <div className="flex justify-between mb-1">
                  ...
                  <span
                    className={`text-sm font-medium ${indicatorProps.textColor}`}
                  >
