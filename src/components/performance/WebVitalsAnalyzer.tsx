...
export function WebVitalsAnalyzer({
  initialData,
}: {
  initialData?: WebVitalMetric[];
}) {
  ...
  const processData = useCallback((data: WebVitalMetric[]) => {
    // Regrouper par page
    const pageGroups = data.reduce(
      (groups: Record<string, WebVitalMetric[]>, item) => {
    ...
    const processed: PageMetrics[] = Object.entries(pageGroups).map(
      ([page, metrics]) => {
        ...
        const aggregatedMetrics = Object.entries(metricGroups).map(
          ([name, values]) => {
            values.sort((a, b) => a - b);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        `Erreur lors du chargement des données: ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
      setLoading(false);
    }
                  ...
                  <div key={metric.metricName} className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <div>
                          {formatMetricValue(metric.metricName, metric.median)}
                        </div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">P75</div>
                        <div>
                          {formatMetricValue(metric.metricName, metric.p75)}
                        </div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">P95</div>
                        <div>
                          {formatMetricValue(metric.metricName, metric.p95)}
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-muted-foreground mb-4 max-w-md">
              Les métriques Web Vitals commenceront à être collectées lorsque
              des utilisateurs visiteront votre site.
            </p>
