...
import { BarChart, CheckCircle, Info, Loader2, SplitIcon, X } from 'lucide-react'
import { 
  Popover, 
  PopoverContent, 
  PopoverTrigger 
    ...
    const handlePerformanceEntry = (entries: PerformanceEntry[]) => {
      for (const entry of entries) {
        ...
        if (
          ...
          (entry.name.includes('worker') || entry.name.includes('filter-') || entry.name.includes('sort-'))
        ) {
          ...
          setWorkersMeasures(prev => {
            ...
            if (existingIndex >= 0) {
              ...
              if (isWithWorker) {
                ...
                newMeasures[existingIndex].withoutWorker = measureEntry.duration
              }
              
              // Calculer l'amélioration si les deux valeurs sont disponibles
              if (newMeasures[existingIndex].withWorker && newMeasures[existingIndex].withoutWorker) {
