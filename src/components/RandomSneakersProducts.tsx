...
export function RandomSneakersProducts() {
    ...
    useEffect(() => {
        const fetchRandomSneakersProducts = async () => {
            try {
                setIsLoading(true)
                const response = await api.fetchProducts({ store_type: "sneakers", limit: "8" })
            ...
            <div className="flex justify-center mt-5 sm:mt-6 md:mt-8 lg:mt-10">
                <Button 
                    ...
                    className="font-geist text-xs tracking-wide uppercase
                        ...
                        h-8 sm:h-9 md:h-10"
                >
                    <Link href="/catalogue?store_type=sneakers" className="flex items-center gap-1.5">
                        Voir tous les sneakers
