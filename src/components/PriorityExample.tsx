...
import {
  ...
  Priority,
  TaskResult,
} from "@/app/contexts/PriorityContext";
...
export default function PriorityExample() {
  ...
  const [results, setResults] = useState<
    Array<{ id: string; type: string; priority: Priority; result: any }>
  >([]);

  // Convertir la Map en tableau pour l'affichage
  useEffect(() => {
    const resultArray = Array.from(completedTasks.entries()).map(
      ([taskId, result]) => {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
            ...
            <div className="space-y-2">
              ...
              <div className="space-x-2">
                <Button
                  size="sm"
                  variant="default"
                  onClick={() => handleAddImageTask("high")}
                >
                  Image
                </Button>
                <Button
                  size="sm"
                  variant="default"
                  onClick={() => handleAddRecommendationTask("high")}
                >
                  Reco
                </Button>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-x-2">
                <Button
                  size="sm"
                  variant="secondary"
                  onClick={() => handleAddImageTask("medium")}
                >
                  Image
                </Button>
                <Button
                  size="sm"
                  variant="secondary"
                  onClick={() => handleAddRecommendationTask("medium")}
                >
                  Reco
                </Button>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-x-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleAddImageTask("low")}
                >
                  Image
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleAddUserPreferencesTask("low")}
                >
                  Préf
                </Button>
              </div>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto">
                {results.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-1">{renderResult(item.result)}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <CardFooter className="text-sm text-muted-foreground">
        Les tâches sont exécutées dans l&apos;ordre de priorité: haute → moyenne
        → basse
      </CardFooter>
    </Card>
  );
