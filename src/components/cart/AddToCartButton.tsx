...
export function AddToCartButton({ productId, name, price, image, disabled, size, color }: AddToCartButtonProps) {
    ...
    const handleAddToCart = async () => {
        ...
        addItem({ id: productId, name, price, quantity: 1, image, size, color });
        toast({
            title: "Produit ajouté",
            description: `${name} (Taille: ${size}, Couleur: ${color}) a été ajouté à votre panier.`,
        });
        ...
        timeline
            .add({
                targets: buttonRef.current,
                scale: [1, 0.95, 1],
                //borderRadius: ['0.25rem', '2rem', '0.25rem'],
                duration: 300,
                easing: 'easeInOutQuad',
            })
            .add({
    ...
    return (
        <div className="relative overflow-visible">
            ...
            <div ref={particlesRef} className={`absolute inset-0 pointer-events-none ${state.showParticles ? 'opacity-100' : 'opacity-0'}`}>
                {[...Array(30)].map((_, i) => (
                    <div
                        key={i}
