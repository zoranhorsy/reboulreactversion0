...
import { X, Filter, RotateCcw, ChevronDown, ChevronUp, Check } from 'lucide-react'
...
export function MobileFilterModal({
  ...
  useEffect(() => {
    ...
    for (const key of filterKeys) {
      if (localFilters[key] && key !== 'sort' && key !== 'view' && key !== 'page' && key !== 'limit') {
        count++
      }
    }
                ...
                <div className="mb-3 text-center">
                  {isLoading ? (
                    ...
                    <p className="text-sm font-medium">
                      <span className="text-primary">{previewCount}</span> produit{previewCount !== 1 ? 's' : ''} correspondant{previewCount !== 1 ? 's' : ''}
                    </p>
                  )}
                </div>
              )}
