...
const convertKidsSizesToStandard = (kidsSizes: KidsSizeChart[]): SizeChart[] => {
...
interface SizeGuideManagerProps {
  ...
  storeType: "adult" | "kids" | "sneakers" | "cpcompany" | "deleted" | undefined;
...
export function SizeGuideManager({ 
  ...
  const useSizeChart = productSizeChart && productSizeChart.length > 0 
    ? productSizeChart 
  ...
  const filteredSizeChart = availableSizes && availableSizes.length > 0
      ...
      <div className="space-y-4">
        <Tabs defaultValue="guide" className="w-full">
          ...
          <TabsContent value="guide" className="space-y-6 pt-4">
            <div className="flex justify-center">
              <Image 
                ...
                onError={(e) => {
                  e.currentTarget.src = 'https://placehold.co/400x200?text=Guide+Chaussures';
                }}
              />
            </div>
            
            <div className="space-y-3">
              <h4 className="font-medium text-sm">Comment mesurer votre pied :</h4>
              <ol className="text-sm space-y-2 list-decimal pl-5">
                <li>Placez une feuille de papier sur le sol contre un mur.</li>
                <li>Tenez-vous debout sur la feuille, le talon contre le mur.</li>
                <li>Tracez le contour de votre pied ou marquez sa longueur maximale.</li>
                <li>Mesurez la distance entre le mur et la marque la plus éloignée.</li>
                <li>Consultez le tableau ci-dessous pour trouver votre pointure.</li>
              </ol>
              
              <p className="text-sm text-muted-foreground mt-2">
                Nous recommandons de mesurer vos pieds en fin de journée, lorsqu&apos;ils sont le plus dilatés.
              </p>
            </div>
            
            <div className="bg-blue-50 border border-blue-100 rounded-md p-3 text-xs text-blue-600">
              <Info className="h-3 w-3 inline-block mr-1" />
              Nos pointures sont basées sur les standards européens (EU). Utilisez le tableau de conversion pour les autres systèmes.
            </div>
          </TabsContent>
          ...
          <TabsContent value="conversion" className="space-y-4 pt-4">
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b">
                    <th className="py-2 px-2 text-left text-xs font-medium">EU</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">US</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">UK</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">CM</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">POUCES</th>
                  </tr>
                </thead>
                <tbody>
                  {defaultSneakerSizes.map((size, index) => {
                    const isAvailable = !availableSizes.length || availableSizes.includes(size.eu);
                    return (
                      <tr 
                        key={index} 
                      ...
                      >
                        <td className="py-2 px-2 text-xs font-medium">{size.eu}</td>
            ...
            <Card className="p-3">
              <h4 className="text-xs font-medium mb-2">Conseils pour le choix de la pointure :</h4>
              <ul className="text-xs space-y-1 list-disc pl-5">
                <li>Si vous êtes entre deux tailles, prenez la plus grande.</li>
                <li>Les personnes ayant un pied large devraient prendre une demi-pointure au-dessus.</li>
                <li>Pour les chaussures de sport, un petit espace (5-10mm) est recommandé au bout.</li>
              </ul>
            </Card>
          </TabsContent>
        </Tabs>
          ...
          <TabsContent value="guide" className="space-y-6 pt-4">
            <div className="flex justify-center">
              <Image 
                ...
                className="max-w-full h-auto"
                onError={(e) => {
                  e.currentTarget.src = 'https://placehold.co/400x300?text=Guide+Enfants';
                }}
              />
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b">
                    <th className="py-2 px-2 text-left text-xs font-medium">ÂGE</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">HAUTEUR (cm)</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">POITRINE (cm)</th>
                    <th className="py-2 px-2 text-left text-xs font-medium">TAILLE (cm)</th>
                  </tr>
                </thead>
                <tbody>
                  {defaultKidsSizes.map((size, index) => {
                    const isAvailable = !availableSizes.length || availableSizes.includes(size.age);
                    return (
                      <tr 
                        key={index} 
                        className={`border-b ${!isAvailable ? 'opacity-50' : ''} ${
                      ...
                      >
                        <td className="py-2 px-2 text-xs font-medium">{size.age}</td>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-md p-3 text-xs text-blue-600">
              <Info className="h-3 w-3 inline-block mr-1" />
              Ces indications sont données à titre indicatif et peuvent varier selon la morphologie de l&apos;enfant.
            </div>
          </TabsContent>
          
          <TabsContent value="calculator" className="pt-4">
            <SizeGuide 
              sizeChart={filteredSizeChart}
              isKidsSizes={true}
            />
          </TabsContent>
        </Tabs>
          ...
          <TabsContent value="guide" className="space-y-6 pt-4">
            <div className="flex justify-center">
              <Image 
                ...
                className="max-w-full h-auto"
                onError={(e) => {
                  e.currentTarget.src = 'https://placehold.co/400x300?text=Guide+Adultes';
                }}
              />
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b">
                    <th className="py-2 px-3 text-left text-xs font-medium">TAILLE</th>
                    <th className="py-2 px-3 text-left text-xs font-medium">POITRINE (cm)</th>
                    <th className="py-2 px-3 text-left text-xs font-medium">TAILLE (cm)</th>
                    <th className="py-2 px-3 text-left text-xs font-medium">HANCHES (cm)</th>
                  </tr>
                </thead>
                <tbody>
                  {defaultAdultSizes.map((size, index) => {
                    const isAvailable = !availableSizes.length || availableSizes.includes(size.size);
                    return (
                      <tr 
                        key={index} 
                        className={`border-b ${!isAvailable ? 'opacity-50' : ''} ${
                      ...
                      >
                        <td className="py-2 px-3 text-xs font-medium">{size.size}</td>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-md p-3 text-xs text-blue-600">
              <Info className="h-3 w-3 inline-block mr-1" />
              Ces informations sont données à titre indicatif. Pour un ajustement parfait, consultez les mesures spécifiques de chaque produit.
            </div>
          </TabsContent>
