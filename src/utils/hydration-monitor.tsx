  ...
  console.error = function (...args) {
    ...
    if (errorString.includes('Hydration') || errorString.includes('mismatch')) {
      ...
      hydrationIssues.push({
        component,
        element: errorString.includes('on <') ? errorString.match(/on <([^>]+)>/)?.[1] || 'Unknown' : 'Unknown',
        mismatch: errorString,
        timestamp: Date.now(),
      })

      // Afficher un avertissement dans la console avec une couleur différente
      console.warn('%c[Moniteur d\'hydratation] Problème détecté:', 'color: #ff9800; font-weight: bold', {
        component,
        message: errorString,
      })
    }
      ...
      const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          if (
            ...
            const timeSinceHydration = performance.now() - (window.__NEXT_HYDRATION_END_TIME || 0)
