...
interface EnfantPageProps {
  searchParams: SearchParams
}

export async function generateMetadata({ searchParams }: EnfantPageProps): Promise<Metadata> {
  const category = searchParams.categories as string | undefined
  const brand = searchParams.brand as string | undefined
  const search = searchParams.search as string | undefined

  let title = "Collection Enfant - Reboul Store"
  let description = "Découvrez notre sélection de vêtements premium pour enfants chez Reboul Store."

  if (category) {
    title = `${category} - Collection Enfant Reboul Store`
    description = `Explorez notre collection de ${category} pour enfants chez Reboul Store.`
  } else if (brand) {
    title = `${brand} - Collection Enfant Reboul Store`
    description = `Découvrez les produits ${brand} pour enfants disponibles chez Reboul Store.`
  } else if (search) {
    title = `Résultats pour "${search}" - Collection Enfant Reboul Store`
    description = `Explorez les résultats de recherche pour "${search}" dans notre collection enfant Reboul Store.`
  }
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "website",
      url: "https://reboul-store.com/enfant",
      images: [
        {
          url: "https://reboul-store.com/og-image.jpg",
          width: 1200,
          height: 630,
          alt: "Reboul Store Collection Enfant",
        },
      ],
    },
  }
...
export default async function EnfantPage({ searchParams }: EnfantPageProps) {
