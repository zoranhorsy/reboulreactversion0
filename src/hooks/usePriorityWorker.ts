...
export function usePriorityWorker() {
  ...
  const [completedTasks, setCompletedTasks] = useState<Map<string, TaskResult>>(new Map());
  const [taskCount, setTaskCount] = useState<number>(0);

  useEffect(() => {
    // Initialisation du worker
    workerRef.current = new Worker(new URL('../workers/priorityWorker.ts', import.meta.url));

    // Gestionnaire de messages
    const messageHandler = (e: MessageEvent) => {
      switch (e.data.type) {
        case 'TASK_COMPLETE':
          setCompletedTasks(prev => new Map(prev).set(e.data.taskId, e.data.result));
          break;
        case 'TASK_ERROR':
  ...
  const getResultsByType = useCallback((type: string): TaskResult[] => {
  ...
  const getResultById = useCallback((taskId: string): TaskResult | undefined => {
    return completedTasks.get(taskId);
  }, [completedTasks]);

  // Ajouter une tâche et traiter immédiatement
  const executeTask = useCallback(async (task: Task): Promise<TaskResult> => {
    return new Promise((resolve, reject) => {
      ...
      const handler = (e: MessageEvent) => {
        ...
        } else if (e.data.type === 'TASK_ERROR' && e.data.taskId === task.id) {
          workerRef.current?.removeEventListener('message', handler);
