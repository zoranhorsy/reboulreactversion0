...
const STORE_STATS = [
  { number: "500+", label: "Marques partenaires" },
...
export function ConceptStore() {
  ...
  const features = [
    ...
    },
    {
      ...
      description: "L'univers REBOUL en images",
      skeleton: (
        <GalleryPreview
          archives={archives.filter((a) => a.category === "store").slice(0, 4)}
        />
      ),
    ...
    },
    {
      ...
      skeleton: <PremiumServices />,
      className:
        "col-span-1 lg:col-span-3 border-b lg:border-r dark:border-neutral-800",
    },
    {
      title: "Marques Partenaires",
      description: "Une sélection des plus grandes maisons",
      skeleton: <BrandsShowcase />,
      className:
        "col-span-1 lg:col-span-3 border-b lg:border-r dark:border-neutral-800",
    },
    {
    ...
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 h-full">
      {ADVANTAGES.map((advantage, index) => (
        <motion.div
          ...
          className="flex flex-col items-center text-center p-3 rounded-lg bg-white dark:bg-neutral-900 shadow-sm border border-neutral-200 dark:border-neutral-800 hover:shadow-md transition-all duration-300"
        >
...
const GalleryPreview = ({ archives }: { archives: Archive[] }) => {
  return (
    <div className="h-full">
      {archives.length > 0 ? (
        <div className="grid grid-cols-2 gap-2 h-full">
          {archives.slice(0, 4).map((archive, idx) => (
            <motion.div
              key={archive.id}
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
