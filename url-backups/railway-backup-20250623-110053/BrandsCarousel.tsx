...
export function BrandsCarousel() {
  ...
  const getBrandLogo = useCallback(
    (brand: Brand) => {
      ...
      let selectedLogo =
        resolvedTheme === "dark" ? brand.logo_light : brand.logo_dark;
  ...
  const handlePrevious = useCallback(() => {
    ...
    setCurrentIndex((prev) => {
      ...
      return newIndex < 0
        ? Math.max(0, brands.length - VISIBLE_BRANDS)
        : newIndex;
    });
      ...
      <section className="w-full min-h-[800px] flex flex-col py-24">
        <div className="flex items-center justify-between gap-4 mb-24">
          <div className="flex items-center gap-12">
            <h2 className="text-3xl font-light uppercase tracking-[0.4em]">
              Nos marques
            </h2>
  ...
  const visibleBrands = [...brands, ...brands].slice(
    currentIndex,
    currentIndex + VISIBLE_BRANDS,
  );

  return (
    <section
      className="w-full min-h-[800px] flex flex-col py-24 relative overflow-hidden"
    ...
    >
      <div className="container mx-auto px-4 md:px-8 flex flex-col">
        <div className="flex items-center justify-between gap-4 mb-24">
          <div className="flex items-center gap-12">
            <h2 className="text-3xl font-light uppercase tracking-[0.4em] text-foreground dark:text-white">
              Nos marques
            </h2>
            <div className="h-px w-48 bg-foreground/10 dark:bg-white/10" />
            <p className="text-base text-foreground/70 dark:text-white/70 tracking-wider">
              Découvrez notre sélection
            </p>
          </div>
          <div className="flex items-center gap-12">
        ...
        <div
          className={`grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 flex-1 transition-all duration-${TRANSITION_DURATION}
          ...
          {visibleBrands.map((brand, index) => (
            ...
            >
              ...
              >
                ...
                {/* Conteneur du texte */}
                <div
                  className="absolute inset-x-0 bottom-0 p-12 flex flex-col items-center justify-end
                                    translate-y-[60%] group-hover:translate-y-0 transition-all duration-700 ease-out"
                >
                  <div className="text-center">
                    <h3
                      className="text-white text-xl font-medium uppercase tracking-[0.4em] mb-4
                    ...
                    {brand.description && (
                      <p
                        className="text-white/90 text-base line-clamp-2 px-6
                    ...
                    <span
                      className="inline-block mt-6 text-sm text-white/90 uppercase tracking-[0.4em]
        ...
        <div className="flex justify-center gap-3 mt-24">
          {Array.from({
            length: Math.ceil(brands.length / VISIBLE_BRANDS),
          }).map((_, index) => (
            <div
              key={index}
              className={cn(
